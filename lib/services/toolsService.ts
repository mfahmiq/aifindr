...
export const toolsService = {
    async getTools(filters?: {
        ...
        if (!includeDetails) {
            ...
            // Public Query: Use * to safe-guard against missing column names, then add joins
            // This is more robust than manual listing
            const relationQuery = `
                *,
            ...
            `
            query = supabase.from('tools').select(relationQuery, { count: 'exact' })
        }
